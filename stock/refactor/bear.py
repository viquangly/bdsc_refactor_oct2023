...
class EveningStar(st.Strategy):
    ...
    def execute(self, price_indexer: st.PriceRetriever) -> st.StrategyResult:
        ...
        low = min(current_day_prices.low, prior_day_prices.low, _2day_ago_prices.low)

        if not all([
            st.has_uptrend(price_indexer),
            prior_day_prices.open > _2day_ago_prices.close > _2day_ago_prices.open,
            current_day_prices.open < prior_day_prices.close,
            current_day_prices.close < _2day_ago_prices.open
        ]):
